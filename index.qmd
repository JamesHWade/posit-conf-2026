...
Advance: the annotations mark this function read-only and closed to the outside world. It returns the prepared article and never fetches the URL. Metadata alone cannot make arbitrary code safe.
:::
...
Advance: ellmer already supports tool calling. Deputy adds a managed run: permissions, hooks, usage limits, and an inspectable result. Here it gets a read-only policy that explicitly allows the article tool, and a limit of six model requests. The allowlist matters. A read-only annotation alone does not authorize a custom tool.
...
::: {.notes}
GEPA looks at the examples it got wrong, reads the feedback from my metric, and proposes better instructions. I supply the examples and the metric that defines a better Orientation.

I care whether the choices help me decide what to read and whether the reasons are supported by their sources. Quotes and document IDs can be checked in code. Usefulness needs a rubric informed by reader judgments, like the reason I typed on the feedback slide. My score_orientation function would combine those and return a score with a short reason. It is application code, not a built-in metric.
...
The code assumes the orient module from the previous slide, an ellmer chat client, curated examples and validation data, and a score_orientation(prediction, row) function built with metric_with_feedback(), returning list(score = , feedback = ) so GEPA can use the feedback text in its reflection step. Evaluation uses evaluate(orient, testset, metric = score_orientation, .llm = chat), then the same call with candidate.

Sources: https://github.com/JamesHWade/dsprrr/blob/76a014f74e8d4a236aa0a0c5dda87de388310cf6/R/compile.R, R/teleprompter-gepa.R, R/evaluate.R, and vignettes/advanced-optimization.Rmd (single-objective GEPA example) at the same revision. compile() takes the program first and the optimizer second. GEPA(metric = ) is the single-objective form; metric_with_feedback() is documented in the GEPA roxygen. Development APIs checked September 13, 2026; GEPA export confirmed September 15, 2026.
-->
